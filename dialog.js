 ...
 *   callback(function):触发者传入dialog的回调函数
 */
...
function SysDialog(option) {
    ...
    this.template = option.template
        ...
        +'                        <b class="close _close" actor="closer"></b> '
        +'                    </div>'
        +'                </td>'
        +'            </tr>'
        +'            <tr>'
...
*    sec(float): 窗口停留时间
*    autoClose(boolean): 是否自动关闭，缺省为true
*    type(string):提示类型 等待wait(缺省),war(警告),suc(成功)
* */
function MsgTip(option) {
    var sec = option.sec || 2,
        autoClose = option.autoClose == null ? true : option.autoClose,
        type = option.type || 'wait',
        template = '<span class="sys_tip sys_' + type + '" actor="content">信息本体</span>';
    var msgDialog = new SysDialog({
        template:template,
...
function fScreenMask(callback) {
    var that = this,
        wh = Math.max($(window.document).height(), $(this.win).height()),
...
function fShowDialog(){
    ...
    for (var i = 0,len = funs.length; i < len; i++) {
        funs[i]['fun'].apply(this, funs[i]['arg'] || []);
    }
...
function fOpenTip() {
    ...
    tip.style.cssText = 'position:fixed;_position:absolute;top:0;left:50%;z-index:10000;';
    tip.innerHTML = '数据加载中...';
...
function dragInit(event,dom) {
    ...
    var minX = $(document).scrollLeft(),
        ...
        maxX = $(window.top).width() - dom.width(),
        maxY = $(window.top).height() - dom.height();
